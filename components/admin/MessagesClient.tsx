        ...
        <div className="flex border-b border-gray-100">
          {tabs.map((tab) => (
            <button key={tab.key} onClick={() => setFilter(tab.key)}
              ...
              {tab.label}
              {tab.count > 0 && (
                <span className={`ml-1 ${filter === tab.key ? 'text-rust' : 'text-gray-300'}`}>({tab.count})</span>
              )}
            </button>
          ))}
        </div>
              ...
              <button key={msg.id} onClick={() => selectMessage(msg)}
                ...
                <div className="flex items-start justify-between gap-2 mb-1">
                  <div className="flex items-center gap-2 min-w-0">
                    <div className={`w-2 h-2 rounded-full shrink-0 ${msg.read ? 'bg-transparent' : 'bg-rust'}`} />
          ...
          <div className="max-w-2xl">
            ...
            <div className="flex gap-2 mb-6">
              <span className={`flex items-center gap-1 text-xs px-2 py-1 font-barlow uppercase tracking-wider
                ${selected.read ? 'bg-green-50 text-green-600' : 'bg-rust/10 text-rust'}`}>
                {selected.read ? <MailOpen className="w-3 h-3" /> : <Mail className="w-3 h-3" />}
                {selected.read ? 'Read' : 'Unread'}
              </span>
              {selected.replied && (
                <span className="flex items-center gap-1 text-xs px-2 py-1 bg-blue-50 text-blue-600 font-barlow uppercase tracking-wider">
            ...
            <div className="bg-white border border-gray-100 p-5">
              ...
              <div className="flex gap-3 mt-3">
                <button onClick={sendReply} disabled={replying || !replyText.trim()}
                  className="flex items-center gap-2 bg-rust text-white font-barlow font-700 text-xs uppercase tracking-widest px-4 py-2 hover:bg-rust-lt transition-colors disabled:opacity-50">
                  {replying ? (
                    <>
                    ...
                    <><Reply className="w-3.5 h-3.5" /> Save Reply</>
                  )}
                </button>
              </div>
            </div>
