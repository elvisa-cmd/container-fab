...
import { Search, Mail, MailOpen, Reply, CheckCheck, X, RefreshCw } from 'lucide-react'
...
export default function MessagesPage() {
  ...
  const loadMessages = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setMessages(list.slice().sort((a, b) => b.created_at.localeCompare(a.created_at)))
  ...
  async function sendReply() {
    ...
    try {
      ...
      if (!res.ok) throw new Error((await res.json()).error ?? 'Failed to save reply')
      ...
      <div className="w-80 shrink-0 border-r border-gray-100 flex flex-col bg-white">
        <div className="p-5 border-b border-gray-100">
          <div className="flex items-center justify-between mb-4">
            <h1 className="font-barlow font-800 text-xl text-steel uppercase tracking-widest">Messages</h1>
            <button
              onClick={loadMessages}
              title="Refresh"
              className="text-gray-300 hover:text-rust transition-colors"
            >
